...
public class DeExifierGUI extends JFrame {
    ...
    private String[] jpgSuffix = { "jpg", "jpeg", "JPG", "JPEG" };
	...
	btnAdd.setIcon(new ImageIcon(DeExifierGUI.class
		.getResource("/reichart/andreas/deexifier/Resources/picBtnJpegAdd.png")));
	...
	btnRemove.setIcon(new ImageIcon(DeExifierGUI.class
		.getResource("/reichart/andreas/deexifier/Resources/picBtnJpegRem.png")));
	...
	buttonPlay.setIcon(new ImageIcon(DeExifierGUI.class
		.getResource("/reichart/andreas/deexifier/Resources/btnPlay.png")));
	...
	btnPrefs.setIcon(new ImageIcon(DeExifierGUI.class
		.getResource("/reichart/andreas/deexifier/Resources/btnPrefs.png")));
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		chooser.setFileFilter(new FileFilter() {
			...
			if (arg0.isDirectory())
			    return true;
			String extensionShort = arg0.getName().substring(arg0.getName().length() - 3);
			String extensionLong = arg0.getName().substring(arg0.getName().length() - 4);

			for (String string : jpgSuffix) {
			    if (string.equals(extensionShort) || string.equals(extensionLong))
				return true;
			}
		...
		if (chooser.showOpenDialog(btnAdd) == JFileChooser.APPROVE_OPTION) {
		    newFiles = chooser.getSelectedFiles();
		    for (File file : newFiles) {
			listModel.addElement(file.getAbsolutePath().toString());
		    }
		    list.setModel(listModel);
		}
	    }
		...
		if (!chkBoxRenamePath.isSelected())
		    savePath = new File(newFiles[0].getParent());
		...
		remover.setParams(newFiles, savePath, qualityValue, recompress, addSuffix, list, progressBar);
